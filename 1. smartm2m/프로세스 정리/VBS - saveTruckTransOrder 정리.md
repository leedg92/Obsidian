...
flowchart TD
    ...
    F -- 아니오 --> Z1
    F -- 예 --> G[주요 정보 추출 (상태, 트럭번호 등)]
    G --> H{기존 COPINO 데이터 존재?}
    H -- 예 --> I{GATE OUT 또는 CANCEL INOUT 상태?}
    I -- 예 --> J[운송 데이터 업데이트 및 팝업 데이터 처리]
    J --> Y[isNeedFcmSend 반환]
    I -- 아니오 --> K{GATE IN 상태 && 논리오류 && 기존오류 아님?}
    K -- 예 --> L{삭제 상태 && 삭제 가능 시간 아님?}
    L -- 예 --> Z1
    L -- 아니오 --> M[일반 정보 체크 및 운송 데이터 업데이트, 팝업 데이터 처리]
    M --> N{DGT신항 EMPTY OUT 특수처리?}
    ...
    P -- 예 --> Z1
    P -- 아니오 --> Q[일반 정보 체크 및 운송 데이터 생성, 팝업 데이터 처리]
    Q --> Y
    Y --> R{트럭번호 동일한 기존 데이터 존재?}
    R -- 예 --> S[예약 취소 처리]
