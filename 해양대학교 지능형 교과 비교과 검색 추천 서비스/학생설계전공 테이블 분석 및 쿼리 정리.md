...
- ### 필요 쿼리 정리(초안)
	...
	4. 교과목 목록 쿼리 (1순위)
		...
		, CASE WHEN C.ORG_LVL = '3' THEN DEPT_CDNM(C.UP_DEPT_CD) ELSE DEPT_CDNM((SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = C.UP_DEPT_CD AND USE_FG = 'Y'))  END AS COLL_NAME  
		, CASE WHEN C.ORG_LVL = '3' THEN C.DEPT_CD ELSE C.UP_DEPT_CD END AS DEPT_CD  
		, CASE WHEN C.ORG_LVL = '3' THEN C.DEPT_NM ELSE DEPT_CDNM((SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = B.DEPT_CD AND USE_FG = 'Y')) END AS DEPT_NAME 
		                  ,  B.DEPT_CD AS MJ_CD  
