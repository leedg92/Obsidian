...
- ### 필요 쿼리 정리(초안)
	1. 학생 개인 정보(이름, 학번, 단과대학, 전공, 휴대폰, EMAIL) ... 이건 이미 짜여져있는게 있을것 같음
		...
		```
	1. '대학 - 학과/학부 - 전공' 셀렉트박스 쿼리 (경우에 따라 '대학 - 전공' 으로만 갈수도있음)  (교육과정 등록하기, 지도교수 선택) 에서 쓸거임	   
	   ```
	   -- 대학 리스트
		...
		FROM (  
		                SELECT  
		                   ...
		                   ,  A.SUBJECT_ENM  
		                    ...
		                    , CASE WHEN C.ORG_LVL = '3' THEN (SELECT DEPT_NM FROM SCO.V_AI_ORG WHERE DEPT_CD = C.UP_DEPT_CD) ELSE (SELECT DEPT_NM FROM SCO.V_AI_ORG WHERE DEPT_CD = (SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = C.UP_DEPT_CD AND USE_FG = 'Y'))  END AS COLL_NAME  
		                    , CASE WHEN C.ORG_LVL = '3' THEN C.DEPT_CD ELSE C.UP_DEPT_CD END AS DEPT_CD  
		                    , CASE WHEN C.ORG_LVL = '3' THEN C.DEPT_NM ELSE (SELECT DEPT_NM FROM SCO.V_AI_ORG WHERE DEPT_CD = (SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = B.DEPT_CD AND USE_FG = 'Y')) END AS DEPT_NAME  
		                  ,  B.DEPT_CD AS MJ_CD  
		                   ,  C.DEPT_NM AS MJ_NAME  
		                  ,  B.YEAR  
		                  ,  B.COM_GRADE  
		                  ,  B.SMT  
		       ...
		       AND (BASE.COLL_CD = '[대학학부전공]' OR BASE.DEPT_CD = '[대학학부전공]' OR BASE.MJ_CD = '[대학학부전공]');
		```
	1. 교수 목록 쿼리 (지도교수)
		```  
		SELECT EMP_NO,  
