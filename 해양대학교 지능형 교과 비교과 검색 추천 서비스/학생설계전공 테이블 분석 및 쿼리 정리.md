	...
	4. 교과목 목록 쿼리 (1순위)
		...
		FROM (  
		                SELECT  
		                   ...
		                   ,  A.SUBJECT_NM  
		                   ,  A.SUBJECT_ENM  
		                    , CASE WHEN C.ORG_LVL = '3' THEN (SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = C.DEPT_CD AND USE_FG = 'Y') ELSE (SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = C.UP_DEPT_CD AND USE_FG = 'Y') END AS COLG_CD  
		                    , CASE WHEN C.ORG_LVL = '3' THEN DEPT_CDNM(C.UP_DEPT_CD) ELSE DEPT_CDNM((SELECT UP_DEPT_CD FROM SCO.V_AI_ORG WHERE DEPT_CD = C.UP_DEPT_CD AND USE_FG = 'Y'))  END AS COLG_NM  
