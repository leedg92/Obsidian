...
- **stdout 출력 형식**:
  - FLUENTD 모드: JSON 형태로 출력
  - KAFKA/DEFAULT 모드: 원본 메시지 그대로 출력
- **관제시스템 노출 제어**: `controlView()` 메서드로 관제시스템에 보여줄 로그만 선택적으로 노출 가능
...
**⚠️ 관제시스템 노출 로그:**
- 관제시스템에 보여줄 로그는 **반드시** `controlView()` 메서드를 호출해야 합니다.
- `controlView()`를 호출하지 않으면 `controlViewYn="N"`으로 설정되며 이는 관제시스템 노출여부의 필터링에 사용됩니다.
- 프로세스의 시작점(req, Kafka 메시지 소비 등)과 최종점(res, Kafka 프로듀싱 등)에만 `controlView()`를 사용하세요.

```java
// 관제시스템에 노출되는 로그 (프로세스 시작/종료)
logger.message("프로세스 시작").logData(data).info().controlView().kafka();
logger.message("프로세스 종료").logData(data).info().controlView().kafka();

// 관제시스템에 노출되지 않는 로그 (일반 로그)
logger.message("작업 완료").logData(data).info().kafka();  // controlViewYn="N"
```
...
public class MyService {
    ...
    public void myMethod() {
        // 기본 메서드 - LOG_MODE에 따라 출력 형식이 달라짐
        logger.info("로그 메시지");
        logger.warn("경고 메시지");
        logger.error("에러 메시지");
...
**stdout 출력 형식:**
- `LOG_MODE=FLUENTD`: JSON 형태로 출력 (Fluentd가 수집)
- `LOG_MODE=KAFKA`: 원본 메시지 그대로 stdout + JSON을 Kafka 전송
- `LOG_MODE` 미설정: 원본 메시지 그대로 stdout
...
logger.message("사용자 로그인")
    ...
    .serviceType("TSS")         // 메서드 체이닝 > 어노테이션 > 환경변수 > null
    .terminal("HJNC")           // 메서드 체이닝 > 환경변수 > null
...
logger.message("주문 처리 완료")
    ...
    .info()
    .kafka();
// stdout: "주문 처리 완료: {"docKey":"DOC001","amount":50000}"
// Kafka: JSON 형태로 전송

// ⚠️ 관제시스템 노출 설정 (프로세스 시작/종료 시 필수)
logger.message("프로세스 시작")
    .logData(logData)
    .serviceType("TSS")
    .info()
    .controlView()             // ⚠️ 관제시스템에 노출하려면 반드시 호출 (controlViewYn="Y")
    .kafka();
...
logger.message("작업 완료")
    .logData(logData)
    .info()
    .kafka();                  // controlViewYn="N" (관제시스템에서 필터링됨)
...
```java
// 기본 메서드 (KAFKA/DEFAULT: 원본 출력, FLUENTD: JSON 출력)
logger.info("로그메시지");
logger.warn("로그메시지");
logger.error("로그메시지");
logger.debug("로그메시지");

// 메서드 체이닝 - Fluentd (JSON 형태로 stdout)
logger.message("로그메시지")
    .logData(mapData)
    .info()
    .fluentd();

// 메서드 체이닝 - Kafka (원본 형태로 stdout + JSON을 Kafka 전송)
logger.message("로그메시지")
    .logData(mapData)
    .warn()
    .kafka();
// stdout: "로그메시지: {logData}"
...
> **참고**: JSON 형태의 출력은 FLUENTD 모드이거나 Kafka로 전송되는 데이터에만 적용됩니다.
> KAFKA/DEFAULT 모드의 stdout은 원본 형태로 출력됩니다.
...
### 기본 메서드 (FLUENTD 모드에서만 JSON 출력)

**입력:** `logger.info("CopinoService [NIFI REV] : CopinoService(a=1,b=2)")`

- **FLUENTD 모드**: JSON 형태로 출력 (메시지가 ":"로 분리됨)
```json
{
  "level": "INFO",
...
```
- **KAFKA/DEFAULT 모드**: 원본 그대로 출력
```
CopinoService [NIFI REV] : CopinoService(a=1,b=2)
```
...
- **traceId**: traceId (메서드 체이닝 `.traceId()` > logData에서 자동 추출, 없으면 빈 문자열). **반드시 pinNo 값만 사용**
- **terminal**: 터미널/IDC 식별자 (메서드 체이닝 > 환경변수 SERVICE_TERMINAL > null)
- **message**: 로그 메시지 (메서드 체이닝으로 설정)
- **controlViewYn**: 관제시스템 노출 여부 (Y 또는 N)
  - **⚠️ 중요**: 관제시스템에 보여줄 로그는 반드시 `controlView()` 메서드를 호출해야 합니다.
  - 기본 메서드(`info()`, `warn()`, `error()`, `debug()`): "N" (관제시스템에서 필터링됨)
  - 메서드 체이닝에서 `controlView()` 호출 시: "Y" (관제시스템에 노출됨)
  - 메서드 체이닝에서 `controlView()` 미호출 시: "N" (관제시스템에서 필터링됨)
  - 프로세스의 시작점(req, Kafka 메시지 소비 등)과 최종점(res, Kafka 프로듀싱 등)에만 사용 권장

## traceId 설정

### 방법 1: 메서드 체이닝으로 직접 주입 (권장)

```java
// traceId를 직접 주입
logger.message("주문 처리")
    .logData(logData)
    .traceId("PIN123456")    // traceId 직접 설정
    .info()
    .kafka();
```

**⚠️ 중요: traceId에는 반드시 pinNo 값만 넣어주세요.**

### 방법 2: logData에서 자동 추출

traceId를 직접 주입하지 않으면, logData가 Map인 경우 다음 우선순위로 자동 추출합니다:
...
**우선순위:** 메서드 체이닝 `.traceId()` > logData에서 자동 추출
...
**마이그레이션 방법 1: 직접 교체 (원본 형태 그대로 출력)**

```java
// Logger를 CustomLogger로 교체
CustomLogger logger = new CustomLogger(getClass());
logger.info("로그메시지: " + 변수값);  // 원본 그대로 출력 (KAFKA/DEFAULT 모드)
```
...
// Kafka: 원본 형태로 stdout + JSON을 Kafka 전송
logger.message("로그메시지")
    .logData(logData)
    .serviceType("TSS")
    .eventType("GATE_IN")
...
- **FLUENTD 모드**: JSON 형태로 출력 (|fluentd| 프리픽스 포함)
- **KAFKA 모드**: 원본 메시지 그대로 stdout + JSON을 Kafka 전송
- **DEFAULT 모드**: 원본 메시지 그대로 stdout
- 기존 로그 시스템에 영향 없음
...
1. **원본 형태로 stdout 출력** - "message: logData" 형태로 출력
2. **JSON 변환** - 로그 데이터를 JSON 형식으로 변환
    ...
    └── main/
        └── java/
            └── com/smartm2m/logging/
                ...
                └── StackTraceHelper.java      # StackTrace 추출 헬퍼
```
