...
| 항목       | 내용                         |
| -------- | -------------------------- |
| 현재 버전    | MariaDB 10.5.9 (Community) |
| OS       | Rocky Linux                |
| DB 규모    | 약 600GB (DB 2~3개)          |
| 10.5 EOL | 2025.06.24 (지원 종료됨)        |

### 1.2 장애 이력

- 2026.01.29 Signal 6 발생 (InnoDB 페이지 corruption)
- 원인: InnoDB Change Buffer 머지 시 페이지 크기 불일치 (MDEV-32132)
- 해당 버그는 10.5.19에서 수정됨 → 현재 10.5.9은 미패치 상태

### 1.3 마이그레이션 필요성

- 10.5 지원 종료로 보안 패치 및 버그 수정 불가
- Signal 6 유발 버그가 수정되지 않은 버전 사용 중
- 장기 안정 운영을 위한 최신 LTS 전환 필요

---

## 2. 마이그레이션 계획 (10.5.9 → 11.8 LTS)

### 2.1 마이그레이션 방식

- **방식: 덤프 & 리스토어 (신규 설치)**
- in-place 업그레이드 대신 채택한 이유:
