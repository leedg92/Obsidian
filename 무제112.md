...
| 총 적재 건수 | 7,134,493건 |
| 일평균 적재량 | **약 119만 건/일** |
| 총 디스크 사용량 | **179 MB** (warehouse + mart 합산) |
...
→ 현재 데이터량 기준으로는 디스크 용량 이슈 없음. 최소 사양(SSD 200GB)으로도 수년간 운영 가능.
...
| 언어 | Java 21 |
| DB 드라이버 | ClickHouse JDBC 0.6.0 |
| 커넥션 풀 | HikariCP (maxPool=5, minIdle=1) |
| 빌드 | Maven |
| DB 접근 | Spring JdbcTemplate (Raw SQL) |
